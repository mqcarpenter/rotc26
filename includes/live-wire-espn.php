...
/**
 * Real stat lines for a drill-down, keyed by ESPN athlete id.
 *
 * MFL's player export carries espn_id, so this matches on that id rather
 * than on a surname -- exact, and immune to the two-Johnsons problem that
 * makes rotc_lw_espn_explain() above return null rather than guess.
 *
 * $teams is the set of NFL teams (MFL codes) actually in the matchup
 * being opened, so a drill-down fetches only the games it needs. This is
 * an explicit user action, unlike the feed, so a handful of summary
 * fetches is a fair trade for a real box score.
 */
const ROTC_LW_STAT_LABELS = [
...
function rotc_lw_espn_statlines(array $mflTeams, ?string $date = null): array {
    $games = rotc_lw_espn_games($date);
    $ids = [];
    ...
    foreach (array_keys($ids) as $gid) {
        ...
        foreach ((array) ($sum['boxscore']['players'] ?? []) as $teamBlock) {
            foreach ((array) ($teamBlock['statistics'] ?? []) as $cat) {
                ...
                $keep   = ROTC_LW_STAT_LABELS[$name] ?? null;
                if (!$keep) continue;
                ...
                foreach ((array) ($cat['athletes'] ?? []) as $a) {
                    ...
                    foreach ($labels as $k => $label) {
                        ...
                        $v = (string) ($stats[$k] ?? '');
                        // Skip empty and plain zeroes -- "0 TD" is noise.
                        if ($v === '' || $v === '0' || $v === '0.0') continue;
                        $bits[] = $v . ' ' . $label;
                    }
                    if (!$bits) continue;
                    // A player can appear in several categories (a RB who
                    // also caught passes); keep them all, longest first so
                    // the primary line leads.
                    $out[$aid][] = implode(' · ', $bits);
                }
            }
        }
    }
    foreach ($out as $aid => $lines) {
        usort($lines, fn($x, $y) => mb_strlen($y) <=> mb_strlen($x));
        $out[$aid] = implode('  |  ', array_slice($lines, 0, 2));
    }
    return $out;
}
